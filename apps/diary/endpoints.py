...
async def get_diary_entries(
    ...
    result = await diary_service.get_all_entries(search=search, mood=mood, page=page, limit=limit)
...
async def create_diary_entry(data: DiaryEntryCreate) -> DiaryEntryResponse:
    try:
        entry = await diary_service.create_entry(data)
        return DiaryEntryResponse.model_validate_from_orm(entry)
    except ObjectNotFound:
        raise NotFound("Mood not found")
...
async def get_diary_entry(entry_id: UUID) -> DiaryEntryResponse:
    try:
        entry = await diary_service.get_entry_by_id(entry_id)
        return DiaryEntryResponse.model_validate_from_orm(entry)
    except ObjectNotFound:
        raise NotFound("Diary entry not found")
...
async def update_diary_entry(entry_id: UUID, data: DiaryEntryUpdate) -> DiaryEntryResponse:
    try:
        entry = await diary_service.update_entry(entry_id, data)
        return DiaryEntryResponse.model_validate_from_orm(entry)
    except ObjectNotFound:
        raise NotFound("Diary entry or mood not found")
...
async def delete_diary_entry(entry_id: UUID) -> dict:
    try:
